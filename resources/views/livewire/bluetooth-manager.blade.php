...
@section('title', 'Bluetooth Manager')
...
<div class="p-4">
    <div class="flex flex-col gap-4">
        ...
        <div class="flex gap-2">
            <button wire:click="start" class="bg-blue-500 text-white px-4 py-2 rounded shadow hover:bg-blue-600 transition">
                Start Scan
            </button>
            <button wire:click="disconnect" class="bg-red-500 text-white px-4 py-2 rounded shadow hover:bg-red-600 transition">
                Disconnect
            </button>
        </div>
        ...
        <div class="mt-4">
            ...
            <div class="flex flex-col gap-2">
                @forelse($devices as $device)
                    <div class="p-3 border rounded-lg flex justify-between items-center bg-white dark:bg-[#161615] dark:border-[#3E3E3A] shadow-sm">
                        <div>
                            <div class="font-bold">{{ $device['name'] }}</div>
                            <div class="text-xs text-gray-500 dark:text-[#A1A09A]">{{ $device['address'] }}</div>
                        </div>
                        <button 
                            wire:click="connect('{{ $device['address'] }}')"

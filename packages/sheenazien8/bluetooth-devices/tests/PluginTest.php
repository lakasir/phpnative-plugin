...
/**
 * Plugin validation tests for BluetoothDevices.
 *
...
describe('Plugin Manifest', function () {
    ...
    it('has required fields', function () {
        ...
        expect($manifest['name'])->toBe('sheenazien8/bluetooth-devices');
        expect($manifest['namespace'])->toBe('BluetoothDevices');
    });
...
describe('Native Code', function () {
    it('has Android Kotlin file', function () {
        $kotlinFile = $this->pluginPath . '/resources/android/BluetoothDevicesFunctions.kt';

        expect(file_exists($kotlinFile))->toBeTrue();

        $content = file_get_contents($kotlinFile);
        expect($content)->toContain('package com.sheenazien8.plugins.bluetooth_devices');
        expect($content)->toContain('object BluetoothDevicesFunctions');
        expect($content)->toContain('BridgeFunction');
    });

    it('has iOS Swift file', function () {
        $swiftFile = $this->pluginPath . '/resources/ios/BluetoothDevicesFunctions.swift';

        expect(file_exists($swiftFile))->toBeTrue();

        $content = file_get_contents($swiftFile);
        expect($content)->toContain('enum BluetoothDevicesFunctions');
        expect($content)->toContain('BridgeFunction');
    });
...
describe('PHP Classes', function () {
    it('has service provider', function () {
        $file = $this->pluginPath . '/src/BluetoothDevicesServiceProvider.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Sheenazien8\BluetoothDevices');
        expect($content)->toContain('class BluetoothDevicesServiceProvider');
    });

    it('has facade', function () {
        $file = $this->pluginPath . '/src/Facades/BluetoothDevices.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Sheenazien8\BluetoothDevices\Facades');
        expect($content)->toContain('class BluetoothDevices extends Facade');
    });

    it('has main implementation class', function () {
        $file = $this->pluginPath . '/src/BluetoothDevices.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Sheenazien8\BluetoothDevices');
        expect($content)->toContain('class BluetoothDevices');
    });
});
